export const productosDB = [
    {
        id: 1,
        nombre: "Cachos Hair Jelly",
        marca: "SKALA (Línea Brasil)",
        categoria: "cabello",
        descripcion: "Gelatina capilar especialmente formulada para definir y realzar todo tipo de rizos. Proporciona definición, hidratación y protección térmica. Ideal para mantener los rizos con aspecto natural, sin encrespamiento y con brillo duradero. #1 en ventas en Brasil.",
        caracteristicas: ["1000g", "Para todo tipo de rizos", "Definición y protección", "Gelatina capilar"],
        imagen: "../img/lote1/producto1.png"
    },
    {
        id: 2,
        nombre: "Óleo de Argan",
        marca: "SKALA (Línea EXPERT)",
        categoria: "cabello",
        descripcion: "Tratamiento nutritivo y revitalizante con aceite de argán para cabellos secos y opacos. Formulado con resistencia capilar mejorada, proporciona nutrición profunda, brillo intenso y suavidad. 100% vegano y sin ingredientes de origen animal.",
        caracteristicas: ["1000g", "Aceite de Argán", "100% Vegano", "Nutrición y revitalización"],
        imagen: "../img/lote1/producto2.png"
    },
    {
        id: 3,
        nombre: "Keratina Vegetal",
        marca: "SKALA (Línea EXPERT)",
        categoria: "cabello",
        descripcion: "Tratamiento revitalizante y fortalecedor con keratina vegetal. Diseñado para reparar, fortalecer y restaurar la estructura capilar. Ideal para cabellos débiles, quebradizos o dañados por procesos químicos. Co-wash.",
        caracteristicas: ["1000g", "Keratina Vegetal", "Revitaliza y fortalece", "Co-wash"],
        imagen: "../img/lote1/producto3.png",
    },
    {
        id: 4,
        nombre: "Óleo de Coco",
        marca: "SKALA (Línea EXPERT)",
        categoria: "cabello",
        descripcion: "Tratamiento nutritivo e hidratante profundo con aceite de coco. Proporciona nutrición intensiva, hidratación profunda y restauración capilar. 100% vegano, ideal para cabellos secos y dañados que necesitan recuperar vitalidad.",
        caracteristicas: ["1000g", "Aceite de Coco", "100% Vegano", "Nutrición e hidratación profunda"],
        imagen: "../img/lote1/producto4.png"
    },
    {
        id: 5,
        nombre: "12 em 1",
        marca: "SKALA",
        categoria: "cabello",
        descripcion: "Tratamiento multibeneficio que ofrece 12 beneficios en un solo producto. Proporciona restauración, brillo, suavidad, hidratación y protección. Fórmula innovadora para cabellos renovados que combina múltiples tratamientos en una sola aplicación.",
        caracteristicas: ["1000g", "12 beneficios", "Restauración y brillo", "Liberado"],
        imagen: "../img/lote1/producto5.png"
    },
    {
        id: 6,
        nombre: "Genetiqs",
        marca: "SKALA (Línea GENETIQS)",
        categoria: "cabello",
        descripcion: "Tratamiento profesional que proporciona fuerza, brillo e hidratación. Formulado con fuerza botánica para cabellos opacos, débiles y sin frizz. Diseñado para transformar cabellos debilitados en cabellos fuertes y radiantes.",
        caracteristicas: ["1000g", "Fuerza botánica", "Brillo e hidratación", "Liberado"],
        imagen: "../img/lote1/producto6.png"
    },
    {
        id: 7,
        nombre: "Coquetel de Frutas Kids",
        marca: "SKALA (Línea EXPERT)",
        categoria: "cabello",
        descripcion: "Tratamiento 2 en 1 especialmente formulado para cabellos infantiles. Con coco y aceite de almendras, limpia y desenreda suavemente mientras nutre el cabello de los niños. Fórmula dermatológicamente testeada, suave y divertida.",
        caracteristicas: ["1000g", "2 en 1", "Coco y almendras", "Para niños", "Liberado"],
        imagen: "../img/lote1/producto7.png"
    },
    {
        id: 8,
        nombre: "Bomba de Biotina",
        marca: "SKALA (Línea EXPERT)",
        categoria: "cabello",
        descripcion: "Tratamiento fortificante con biotina para cabellos débiles y quebradizos. Proporciona nutrición, fortalecimiento y crecimiento saludable del cabello. La biotina ayuda a mejorar la estructura capilar desde la raíz hasta las puntas.",
        caracteristicas: ["1000g", "Biotina", "Nutrición y fortalecimiento", "Liberado"],
        imagen: "../img/lote1/producto8.png"
    },
    {
        id: 9,
        nombre: "Cacau",
        marca: "SKALA (Línea Brasil)",
        categoria: "cabello",
        descripcion: "Tratamiento con cacao que proporciona fuerza y brillo al cabello. Enriquecido con las propiedades antioxidantes del cacao, nutre profundamente y protege el cabello, dejándolo suave, brillante y saludable con un aroma delicioso.",
        caracteristicas: ["1000g", "Cacao", "Fuerza y brillo", "Hidratación"],
        imagen: "../img/lote1/producto9.png"
    },
    {
        id: 10,
        nombre: "Divino Putinho #Influencer",
        marca: "SKALA (Línea EXPERT)",
        categoria: "cabello",
        descripcion: "Tratamiento 2 en 1 hidratante libre de sulfatos. Crema multiuso que limpia y trata en un solo paso, ideal para mantener rizos definidos y cabello hidratado. Fórmula moderna diseñada para el cuidado capilar actual.",
        caracteristicas: ["1000g", "2 en 1", "Libre de sulfato", "Hidratante", "Liberado"],
        imagen: "../img/lote1/producto10.png"
    },
    {
        id: 11,
        nombre: "Creme 3em1 Glicólico",
        marca: "SKALA (Línea EXPERT)",
        categoria: "cabello",
        descripcion: "Tratamiento 3 en 1 con ácido glicólico que funciona como tratamiento, súper acondicionador y crema para peinar. Con complejo 6% glicolizado que proporciona fuerza y brillo intenso. #1 en Brasil. Fórmula innovadora que cuida y transforma el cabello en un solo paso.",
        caracteristicas: ["1000g", "3 en 1", "Ácido Glicólico", "6% Complejo", "Liberado"],
        imagen: "../img/lote2/producto11.png"
    },
    {
        id: 12,
        nombre: "Reacionados Força e Brilho",
        marca: "SKALA (Línea EXPERT)",
        categoria: "cabello",
        descripcion: "Tratamiento 2 en 1 de fuerza y brillo especialmente diseñado para cabellos reaccionados (con tendencia a reacciones). Con extracto mixto de açaí, aceite de coco y buriti. Crema para peinar y acondicionador que fortalece y da brillo. Para cabellos con puntas abiertas o porosos.",
        caracteristicas: ["1000g", "2 en 1", "Açaí y Buriti", "Fuerza y brillo", "Liberado"],
        imagen: "../img/lote2/producto12.png"
    },
    {
        id: 13,
        nombre: "Poder Desarmante",
        marca: "SKALA (Línea EXPERT)",
        categoria: "cabello",
        descripcion: "Tratamiento 2 en 1 con aceite de argán y control vegetal D-Pantenol que controla y desenreda el cabello. Crema para peinar y acondicionador ideal para cabellos rebeldes y difíciles de manejar. Sin frizz, proporcionando suavidad y manejabilidad extrema.",
        caracteristicas: ["1000g", "2 en 1", "Aceite de Argán", "D-Pantenol", "Liberado Free", "Co-wash"],
        imagen: "../img/lote2/producto13.png"
    },
    {
        id: 14,
        nombre: "Divina Cor",
        marca: "SKALA (Línea EXPERT)",
        categoria: "cabello",
        descripcion: "Tratamiento 2 en 1 especialmente formulado para cabellos teñidos. Con proteínas de arroz, vitamina E, extracto de uva y aceite de coco. Protege el color, proporciona restauración y brillo. Mantiene la intensidad del color por más tiempo.",
        caracteristicas: ["1000g", "2 en 1", "Para cabello teñido", "Protección del color", "Liberado"],
        imagen: "../img/lote2/producto14.png"
    },
    {
        id: 15,
        nombre: "Lama Negra Activated Charcoal",
        marca: "SKALA (Línea EXPERT)",
        categoria: "cabello",
        descripcion: "Tratamiento purificante con carbón activado que proporciona maciez (suavidad) y brillo. La lama negra (lodo negro) limpia profundamente el cuero cabelludo, eliminando impurezas y residuos. Ideal para cabellos con exceso de oleosidad y para realizar detox capilar. 100% vegano.",
        caracteristicas: ["1000g", "Carbón Activado", "Maciez y brillo", "100% Vegano", "Liberado Free"],
        imagen: "../img/lote2/producto15.png"
    },
    {
        id: 16,
        nombre: "Frutástica! Uva Brilho Extremo",
        marca: "SKALA (Línea Frutástica!)",
        categoria: "cabello",
        descripcion: "Tratamiento 2 en 1 con extracto de uva que proporciona brillo extremo. Crema para peinar y acondicionador enriquecido con antioxidantes naturales de la uva. Ideal para cabellos opacos y sin brillo. Hidratación intensa y protección. #1 en Brasil.",
        caracteristicas: ["1000g", "2 en 1", "Extracto de Uva", "Brillo extremo", "Hidratación", "Liberado"],
        imagen: "../img/lote2/producto16.png"
    },
    {
        id: 17,
        nombre: "Frutástica! Morango Nutrição e Brilho",
        marca: "SKALA (Línea Frutástica!)",
        categoria: "cabello",
        descripcion: "Tratamiento 2 en 1 con extracto de fresa que proporciona nutrición y brillo. Con vitaminas y antioxidantes de la fresa, nutre profundamente y deja el cabello suave y brillante. Ideal para cabellos secos y sin brillo. Aroma delicioso a fresa fresca. #1 en Brasil.",
        caracteristicas: ["1000g", "2 en 1", "Extracto de Fresa", "Nutrición y brillo", "Liberado"],
        imagen: "../img/lote2/producto17.png"
    },
    {
        id: 18,
        nombre: "Banana e Bacuri",
        marca: "SKALA (Línea Brasil)",
        categoria: "cabello",
        descripcion: "Tratamiento 2 en 1 con banana y bacuri que proporciona reparación y brillo. Combina las propiedades nutritivas de la banana con el aceite de bacuri amazónico. Repara, nutre y da brillo tropical al cabello. Fórmula 100% tropical para cabellos dañados.",
        caracteristicas: ["1000g", "2 en 1", "Banana y Bacuri", "Reparación y brillo", "Liberado"],
        imagen: "../img/lote2/producto18.png"
    },
    {
        id: 19,
        nombre: "Maracujá e Óleo de Patauá",
        marca: "SKALA (Línea Brasil)",
        categoria: "cabello",
        descripcion: "Tratamiento con maracuyá y aceite de patauá amazónico. Crema nutritiva de tratamiento que combina las propiedades calmantes del maracuyá con el aceite de patauá rico en omega. Nutre, hidrata y restaura el cabello naturalmente. Fórmula inspirada en ingredientes brasileños. #1 en Brasil.",
        caracteristicas: ["1000g", "Maracuyá y Patauá", "Nutrición tropical", "Leave-in"],
        imagen: "../img/lote2/producto19.png"
    },
    {
        id: 20,
        nombre: "Dona Skala",
        marca: "SKALA (Línea EXPERT)",
        categoria: "cabello",
        descripcion: "Tratamiento 2 en 1 con manteca de karité, pantenol, aceite de coco, queratina y ceramidas. Fórmula completa que nutre, repara y protege el cabello. Combinación de ingredientes premium para un tratamiento completo y efectivo. Cabello suave, brillante y saludable.",
        caracteristicas: ["1000g", "2 en 1", "Karité y Ceramidas", "Queratina y Pantenol", "Liberado"],
        imagen: "../img/lote2/producto20.png"
    },
    {
        id: 21,
        nombre: "Caju e Murumuru",
        marca: "SKALA (Línea Brasil)",
        categoria: "cabello",
        descripcion: "Tratamiento con cajú y manteca de murumuru amazónica que proporciona resistencia y brillo. La manteca de murumuru es conocida por sus propiedades hidratantes y protectoras. Fórmula tropical que fortalece y nutre profundamente el cabello. Ingredientes nativos de Brasil.",
        caracteristicas: ["1000g", "Cajú y Murumuru", "Resistencia y brillo", "Liberado"],
        imagen: "../img/lote2/producto21.png"
    },
    {
        id: 22,
        nombre: "Kit 12 em 1 - Fios Renovados",
        marca: "SKALA",
        categoria: "cabello",
        descripcion: "Kit precio especial que incluye shampoo 325ml y acondicionador 325ml de la línea 12 em 1. Proporciona restauración y brillo para cabellos renovados. 100% vegano sin ingredientes de origen animal. 12 beneficios en un solo sistema de cuidado capilar completo.",
        caracteristicas: ["Shampoo 325ml", "Acondicionador 325ml", "12 beneficios", "100% Vegano", "Precio especial"],
        imagen: "../img/lote3/producto22.png"
    },
    {
        id: 23,
        nombre: "Kit Frutástica! Uva - Brilho Extremo",
        marca: "SKALA (Línea Frutástica!)",
        categoria: "cabello",
        descripcion: "Kit precio especial de la línea Uva con brillo extremo. Incluye shampoo 325ml y acondicionador 325ml con extracto de uva. Sistema completo para proporcionar brillo intenso y duradero. #1 en Brasil. Precio especial para tratamiento completo.",
        caracteristicas: ["Shampoo 325ml", "Acondicionador 325ml", "Extracto de Uva", "Brillo extremo", "#1 Brasil"],
        imagen: "../img/lote3/producto23.png"
    },
    {
        id: 24,
        nombre: "Kit Bomba de Vitaminas - Crecimiento Espetacular",
        marca: "SKALA (Línea EXPERT)",
        categoria: "cabello",
        descripcion: "Kit precio especial con bomba de vitaminas y ácido hialurónico para crecimiento espectacular. Incluye shampoo 325ml y acondicionador 325ml. Fórmula enriquecida que estimula el crecimiento saludable del cabello. 100% vegano sin ingredientes de origen animal.",
        caracteristicas: ["Shampoo 325ml", "Acondicionador 325ml", "Ácido Hialurónico", "Vitaminas", "100% Vegano"],
        imagen: "../img/lote3/producto24.png"
    },
    {
        id: 25,
        nombre: "Kit Frutástica! Morango - Nutrição e Brilho",
        marca: "SKALA (Línea Frutástica!)",
        categoria: "cabello",
        descripcion: "Kit precio especial de la línea Morango (Fresa) con nutrición y brillo. Incluye shampoo 325ml y acondicionador 325ml con extracto de fresa. Sistema completo que nutre profundamente y proporciona brillo natural. #1 en Brasil. Aroma delicioso a fresa.",
        caracteristicas: ["Shampoo 325ml", "Acondicionador 325ml", "Extracto de Fresa", "Nutrición y brillo", "#1 Brasil"],
        imagen: "../img/lote3/producto25.png"
    },
    {
        id: 26,
        nombre: "Kit Frutástica! Coco - Nutrição Poderosa",
        marca: "SKALA (Línea Frutástica!)",
        categoria: "cabello",
        descripcion: "Kit precio especial de la línea Coco con nutrición poderosa. Incluye shampoo 325ml y acondicionador 325ml con aceite de coco. Sistema completo de nutrición intensiva para cabellos secos y dañados. 100% vegano. Precio especial.",
        caracteristicas: ["Shampoo 325ml", "Acondicionador 325ml", "Aceite de Coco", "Nutrición poderosa", "100% Vegano"],
        imagen: "../img/lote3/producto26.png"
    },
    {
        id: 27,
        nombre: "Kit Frutástica! Melancia - Revitalização de Cachos",
        marca: "SKALA (Línea Frutástica!)",
        categoria: "cabello",
        descripcion: "Kit precio especial de la línea Melancia (Sandía) para revitalización de rizos. Incluye shampoo 325ml y acondicionador 325ml con extracto de sandía. Sistema completo que revitaliza, define y da vida a los rizos. Hidratación refrescante. #1 en Brasil.",
        caracteristicas: ["Shampoo 325ml", "Acondicionador 325ml", "Extracto de Sandía", "Revitalización de rizos", "#1 Brasil"],
        imagen: "../img/lote3/producto27.png"
    },
    {
        id: 28,
        nombre: "Kit Óleo de Argan - Nutrição e Revitalização",
        marca: "SKALA (Línea EXPERT)",
        categoria: "cabello",
        descripcion: "Kit precio especial con aceite de argán para nutrición y revitalización. Incluye shampoo sin sal 325ml y acondicionador 325ml. Sistema completo 100% vegano que nutre profundamente y revitaliza cabellos secos. Uso capilar profesional.",
        caracteristicas: ["Shampoo sin sal 325ml", "Acondicionador 325ml", "Aceite de Argán", "100% Vegano", "Precio especial"],
        imagen: "../img/lote3/producto28.png"
    },
    {
        id: 29,
        nombre: "Kit Frutástica! Banana - Reparação Surpreendente",
        marca: "SKALA (Línea Frutástica!)",
        categoria: "cabello",
        descripcion: "Kit precio especial de la línea Banana con reparación sorprendente. Incluye shampoo 325ml y acondicionador 325ml con extracto de banana. Sistema completo que repara profundamente cabellos dañados. Rico en potasio y vitaminas. #1 en Brasil.",
        caracteristicas: ["Shampoo 325ml", "Acondicionador 325ml", "Extracto de Banana", "Reparación sorprendente", "#1 Brasil"],
        imagen: "../img/lote3/producto29.png"
    },
    {
        id: 30,
        nombre: "Creme para Pentear Super Desembaraço",
        marca: "Salon Line",
        categoria: "cabello",
        descripcion: "Crema para peinar super desenredante con definición y desembaraço. Formulada para facilitar el peinado de cabellos rizados, crespos y difíciles de manejar. Proporciona definición, suavidad y brillo. Fácil de aplicar y distribuir. Presentación en botella de 1 litro.",
        caracteristicas: ["1L", "Super desenredante", "Definición", "Para rizos y crespos"],
        imagen: "../img/lote3/producto30.png"
    },
    {
        id: 31,
        nombre: "Creme para Pentear Super Controle",
        marca: "Salon Line",
        categoria: "cabello",
        descripcion: "Crema para peinar super control con definición y reducción de volumen. Ideal para cabellos ondulados, rizados y crespos que necesitan control y disciplina. Controla el frizz, reduce el volumen y proporciona definición duradera. Presentación en botella de 1 litro.",
        caracteristicas: ["1L", "Super control", "Reduce volumen", "Definición y antifrizz"],
        imagen: "../img/lote3/producto31.png"
    },
    {
        id: 32,
        nombre: "Kit Vitamina C + Colágeno Vegetal - Reparação e Brilho",
        marca: "SKALA (Línea EXPERT)",
        categoria: "cabello",
        descripcion: "Kit precio especial con vitamina C y colágeno vegetal para reparación y brillo. Incluye shampoo 325ml y acondicionador 325ml. Acción antioxidante que repara, fortalece y proporciona brillo intenso. 100% vegano sin ingredientes de origen animal.",
        caracteristicas: ["Shampoo 325ml", "Acondicionador 325ml", "Vitamina C", "Colágeno Vegetal", "100% Vegano"],
        imagen: "../img/lote3/producto32.png"
    },
    {
        id: 33,
        nombre: "Creme para Pentear Super Volumão",
        marca: "Salon Line",
        categoria: "cabello",
        descripcion: "Crema para peinar super volumão (super volumen) con definición y volumen prolongado. Ideal para cabellos que necesitan más cuerpo, volumen y movimiento. Proporciona volumen natural sin pesar, con definición y brillo. Presentación en botella de 1 litro.",
        caracteristicas: ["1L", "Super volumen", "Definición volumosa", "Cabello con cuerpo"],
        imagen: "../img/lote3/producto33.png"
    },
    {
        id: 34,
        nombre: "Creme para Pentear Definição Ultra Volumê",
        marca: "Salon Line",
        categoria: "cabello",
        descripcion: "Crema para peinar en frasco con definición ultra volumen. Proporciona nutrición, movimiento y definición volumosa con efecto antifrizz. Ideal para cabellos finos, ondulados y rizados que necesitan volumen y definición. Fórmula que realza los rizos y proporciona cuerpo. Presentación en frasco de 1kg.",
        caracteristicas: ["1kg", "Ultra volumen", "Definición", "Antifrizz", "Nutrición y movimiento"],
        imagen: "../img/lote3/producto34.png"
    },
    {
        id: 35,
        nombre: "Creme para Pentear Super Volumão (Pump)",
        marca: "Salon Line",
        categoria: "cabello",
        descripcion: "Crema para peinar super volumão en presentación con bomba dispensadora (pump) para mayor comodidad y facilidad de aplicación. Hidratación intensa con volumen prolongado. Definición y antifrizz para cabellos con movimiento y cuerpo. Sistema de 10 beneficios. Presentación de 1 litro con dispensador.",
        caracteristicas: ["1L", "Con bomba dispensadora", "Super volumen", "10 beneficios", "Hidratación intensa"],
        imagen: "../img/lote3/producto35.png"
    },
    {
        id: 36,
        nombre: "Creme para Pentear Definição Máxima Liberada",
        marca: "Salon Line",
        categoria: "cabello",
        descripcion: "Crema para peinar en frasco con definición máxima liberada. Hidratación intensa, definición y brillo para rizos y crespos. Fórmula antifrizz que proporciona definición perfecta sin dejar el cabello duro o pesado. Para cabellos liberados (técnica low-poo/no-poo). Presentación en frasco de 1kg.",
        caracteristicas: ["1kg", "Definición máxima", "Liberada", "Hidratación intensa", "Antifrizz"],
        imagen: "../img/lote3/producto36.png"
    },
    {
        id: 37,
        nombre: "Gelatina + Creme para Pentear - Definição Extraordinária",
        marca: "Salon Line",
        categoria: "cabello",
        descripcion: "Crema para peinar enriquecida con gelatina que proporciona definición extraordinaria. Hidratación, superdefinición y fijación flexible con textura gel-creme. Ideal para cabellos 2abc, 3abc y 4abc. Proporciona rizos definidos sin dejar el cabello duro o crujiente.",
        caracteristicas: ["1kg", "Gelatina", "Definición extraordinaria", "Textura gel-creme", "Para todo tipo de rizos"],
        imagen: "../img/lote4/producto37.png"
    },
    {
        id: 38,
        nombre: "S.O.S Hidratação - Máscara Concentrada Aceite de Oliva",
        marca: "Salon Line",
        categoria: "cabello",
        descripcion: "Máscara concentrada S.O.S Hidratação con aceite de oliva para tratamiento intensivo. Proporciona hidratación profunda, nutrición y restauración para cabellos resecos, porosos y dañados. Fórmula concentrada de acción rápida y eficaz. Restaura la fibra capilar desde el interior.",
        caracteristicas: ["1kg", "Aceite de Oliva", "Hidratación S.O.S", "Máscara concentrada", "Restauración intensa"],
        imagen: "../img/lote4/producto38.png"
    },
    {
        id: 39,
        nombre: "S.O.S Hidratação - Máscara Concentrada Óleo de Rícino",
        marca: "Salon Line",
        categoria: "cabello",
        descripcion: "Máscara concentrada S.O.S Hidratação con aceite de ricino para tratamiento profundo. El aceite de ricino fortalece, estimula el crecimiento y proporciona hidratación intensa. Ideal para cabellos debilitados, con caída o que necesitan fortalecimiento. Fórmula concentrada de alta eficacia.",
        caracteristicas: ["1kg", "Aceite de Ricino", "Hidratación S.O.S", "Fortalecimiento", "Máscara concentrada"],
        imagen: "../img/lote4/producto39.png"
    },
    {
        id: 40,
        nombre: "S.O.S Cachos - Óleo de Rícino e Queratina",
        marca: "Salon Line",
        categoria: "cabello",
        descripcion: "Crema para peinar S.O.S Cachos con aceite de ricino y queratina para reparación y antifrizz. Tratamiento especializado que repara, fortalece y define rizos dañados. La combinación de ricino y queratina restaura la estructura capilar y proporciona definición duradera.",
        caracteristicas: ["1kg", "Aceite de Ricino", "Queratina", "Reparación y antifrizz", "S.O.S Cachos"],
        imagen: "../img/lote4/producto40.png"
    },
    {
        id: 41,
        nombre: "Hidra Babosa - Queratina e Glicerina Vegetal",
        marca: "Salon Line",
        categoria: "cabello",
        descripcion: "Tratamiento Hidra con extracto de babosa (aloe vera), queratina y glicerina vegetal. Proporciona hidratación intensa, restauración profunda para cabellos dañados y quebradizos. El aloe vera calma y repara, mientras la queratina fortalece. Presentación en botella con dispensador de 1 litro.",
        caracteristicas: ["1L", "Babosa (Aloe Vera)", "Queratina", "Glicerina Vegetal", "Restauración intensa"],
        imagen: "../img/lote4/producto41.png"
    },
    {
        id: 42,
        nombre: "S.O.S Hidratação - Máscara Concentrada Queratina e Colágeno",
        marca: "Salon Line",
        categoria: "cabello",
        descripcion: "Máscara concentrada S.O.S Hidratação con queratina y colágeno para hidratación intensiva. Reconstruye la fibra capilar, repara cabellos dañados y proporciona fuerza y elasticidad. Tratamiento de shock para cabellos muy dañados o procesados químicamente.",
        caracteristicas: ["1kg", "Queratina", "Colágeno", "Hidratación S.O.S", "Máscara concentrada"],
        imagen: "../img/lote4/producto42.png"
    },
    {
        id: 43,
        nombre: "#TodeCacho Ativador de Cachos - Redução Volumosa",
        marca: "Salon Line",
        categoria: "cabello",
        descripcion: "Activador de rizos #TodeCacho con reducción volumosa. Proporciona definición, control de volumen y activación derizos naturales. Ideal para rizos tipo 3 y 4 que necesitan definición sin frizz y control de volumen. Fórmula ligera que no pesa. Presentación de 300ml.",
        caracteristicas: ["300ml", "Activador de rizos", "Reducción volumosa", "Definición sin frizz"],
        imagen: "../img/lote4/producto43.png"
    },
    {
        id: 44,
        nombre: "S.O.S Cachos - Recarga de Queratina",
        marca: "Salon Line (Línea S.O.S Cachos)",
        categoria: "cabello",
        descripcion: "Activador de rizos con recarga de queratina para reparación y brillo. Tratamiento especializado que reconstruye la fibra capilar, repara daños y proporciona definición intensa. La queratina penetra profundamente restaurando la estructura del cabello. Presentación de 500ml.",
        caracteristicas: ["500ml", "Recarga de Queratina", "Reparación y brillo", "Definición intensa"],
        imagen: "../img/lote4/producto44.png"
    },
    {
        id: 45,
        nombre: "S.O.S Cachos - Ácido Hialurônico",
        marca: "Salon Line (Línea S.O.S Cachos)",
        categoria: "cabello",
        descripcion: "Activador de rizos con ácido hialurónico para hidratación y sellado. El ácido hialurónico retiene la hidratación, proporciona elasticidad y define los rizos. Tratamiento que sella las cutículas, combate el frizz y mantiene los rizos hidratados por más tiempo. Presentación de 500ml.",
        caracteristicas: ["500ml", "Ácido Hialurónico", "Hidratación y sellado", "Definición duradera"],
        imagen: "../img/lote4/producto45.png"
    },
    {
        id: 46,
        nombre: "S.O.S Cachos - Aceite de Oliva",
        marca: "Salon Line (Línea S.O.S Cachos)",
        categoria: "cabello",
        descripcion: "Activador de rizos con aceite de oliva para nutrición y antifrizz. El aceite de oliva nutre profundamente, proporciona brillo natural y suavidad extrema. Define rizos mientras protege contra el frizz y factores externos. Textura ligera que no deja el cabello pesado. Presentación de 500ml.",
        caracteristicas: ["500ml", "Aceite de Oliva", "Nutrición y antifrizz", "Brillo natural"],
        imagen: "../img/lote4/producto46.png"
    },
    {
        id: 47,
        nombre: "#TodeCacho Ativador de Cachos - Coco",
        marca: "Salon Line",
        categoria: "cabello",
        descripcion: "Activador de rizos #TodeCacho con aceite de coco. Define intensamente, proporciona hidratación profunda y restaura el brillo natural. El aceite de coco penetra la fibra capilar nutriendo desde el interior. Ideal para rizos secos y sin vida. Presentación de 300ml.",
        caracteristicas: ["300ml", "Aceite de Coco", "Definición intensa", "Hidratación y brillo"],
        imagen: "../img/lote4/producto47.png"
    },
    {
        id: 48,
        nombre: "Hidra Ativador de Cachos - Hidratação Intensa + Movimento",
        marca: "Salon Line",
        categoria: "cabello",
        descripcion: "Activador de rizos Hidra 2 en 1 con D-Pantenol para hidratación intensa y movimiento. Fórmula que hidrata, define y proporciona movimiento natural a los rizos. El D-Pantenol retiene la humedad y mejora la elasticidad. Rizos sueltos, definidos y con vida. Presentación de 300ml.",
        caracteristicas: ["300ml", "2 en 1", "D-Pantenol", "Hidratación intensa", "Movimiento natural"],
        imagen: "../img/lote4/producto48.png"
    },
    {
        id: 49,
        nombre: "#TodeCacho Spray Finalizador - Toque de Seda",
        marca: "Salon Line",
        categoria: "cabello",
        descripcion: "Spray finalizador #TodeCacho con toque de seda para finalizar el peinado de rizos. Proporciona brillo intenso, fijación ligera, control del frizz y protección. Sella las cutículas dejando los rizos con acabado sedoso y natural. Fórmula bifásica que nutre y define. Presentación de 250ml.",
        caracteristicas: ["250ml", "Spray finalizador", "Toque de seda", "Brillo y fijación", "Antifrizz"],
        imagen: "../img2/lote5/producto49.png"
    },
    {
        id: 50,
        nombre: "Kit Hidra Ceramidas - Shampoo y Acondicionador",
        marca: "Salon Line",
        categoria: "cabello",
        descripcion: "Kit Hidra con ceramidas, D-Pantenol y aceite de jojoba. Incluye shampoo 300ml y acondicionador 300ml. Sistema completo de nutrición para cabellos frágiles y opacos. Las ceramidas reparan la fibra capilar, el D-Pantenol hidrata y el aceite de jojoba nutre profundamente.",
        caracteristicas: ["Shampoo 300ml", "Acondicionador 300ml", "Ceramidas", "D-Pantenol", "Aceite de Jojoba"],
        imagen: "../img2/lote5/producto50.png"
    },
    {
        id: 51,
        nombre: "S.O.S Bomba Original - Shampoo Ultra Hidratação Reconstrutora",
        marca: "Salon Line (Línea S.O.S Bomba)",
        categoria: "cabello",
        descripcion: "Shampoo S.O.S Bomba Original profesional con ultra hidratación reconstructora. Fórmula con 10x más hidratación que proporciona tratamiento profesional intensivo. Reconstruye, hidrata y restaura cabellos muy dañados. Ideal para tratamiento de shock. Presentación de 300ml.",
        caracteristicas: ["300ml", "10x más hidratación", "Ultra hidratación", "Reconstructor", "Profesional"],
        imagen: "../img2/lote5/producto51.png"
    },
    {
        id: 52,
        nombre: "Meu Lisinho Kids - Creme para Pentear Imaginação e Diversão",
        marca: "Salon Line (Línea Kids)",
        categoria: "cabello",
        descripcion: "Crema para peinar Meu Lisinho Kids con fórmula especial para niños. Imaginación y diversión para desenredar y cuidar diariamente cabellos infantiles sin nós (nudos). Fórmula suave, segura y testeada dermatológicamente. Facilita el peinado haciendo el momento divertido. Presentación de 300ml.",
        caracteristicas: ["300ml", "Para niños", "Desenredante", "Sin nudos", "Fórmula suave"],
        imagen: "../img2/lote5/producto52.png"
    },
    //53 se elimina, estaba repetido.
    {
        id: 54,
        nombre: "S.O.S Bomba - Tônico Fortalecedor Crescimento",
        marca: "Salon Line (Línea S.O.S Bomba)",
        categoria: "cabello",
        descripcion: "Tónico fortalecedor S.O.S Bomba para crecimiento capilar. Estimula el crecimiento, fortalece la raíz y reduce la caída. Fórmula concentrada que revitaliza el cuero cabelludo y estimula los folículos pilosos. Aplicación directa en el cuero cabelludo. Presentación de 100ml.",
        caracteristicas: ["100ml", "Tónico fortalecedor", "Estimula crecimiento", "Reduce caída", "Aplicación directa"],
        imagen: "../img2/lote5/producto54.png"
    },
    {
        id: 55,
        nombre: "S.O.S Bomba Original - Creme para Pentear Ultra Hidratação",
        marca: "Salon Line (Línea S.O.S Bomba)",
        categoria: "cabello",
        descripcion: "Crema para peinar S.O.S Bomba Original con ultra hidratación profesional. Fórmula con 13x más hidratación que nutre, restaura y proporciona brillo extremo. Tratamiento profesional leave-in que sella las cutículas y protege contra daños. Presentación de 300ml.",
        caracteristicas: ["300ml", "13x más hidratación", "Ultra hidratación", "Profesional", "Leave-in"],
        imagen: "../img2/lote5/producto55.png"
    },
    {
        id: 56,
        nombre: "S.O.S Bomba - Tônico Fortalecedor Antiqueda",
        marca: "Salon Line (Línea S.O.S Bomba)",
        categoria: "cabello",
        descripcion: "Tónico fortalecedor S.O.S Bomba antiqueda para combatir y prevenir la caída del cabello. Fortalece la raíz, estimula el cuero cabelludo y revitaliza los folículos. Fórmula concentrada de acción profunda que reduce significativamente la caída. Presentación de 100ml.",
        caracteristicas: ["100ml", "Tónico antiqueda", "Fortalece raíz", "Estimula folículos", "Aplicación directa"],
        imagen: "../img2/lote5/producto56.png"
    },
    {
        id: 57,
        nombre: "Cachos dos Sonhos Multy Maravilhas 3em1",
        marca: "Salon Line (Línea Cachos dos Sonhos)",
        categoria: "cabello",
        descripcion: "Tratamiento 3 en 1 Cachos dos Sonhos Multy Maravilhas. Crema de tratamiento, súper acondicionador y crema para peinar en un solo producto. Proporciona banho de brilho (baño de brillo), nutrición y textura extrema. Multiuso para todo tipo de rizos. Presentación de 1kg.",
        caracteristicas: ["1kg", "3 en 1", "Banho de brilho", "Nutrición", "Textura extrema"],
        imagen: "../img2/lote5/producto57.png"
    },
    {
        id: 58,
        nombre: "Cachos dos Sonhos Multy Milagres 3em1",
        marca: "Salon Line (Línea Cachos dos Sonhos)",
        categoria: "cabello",
        descripcion: "Tratamiento 3 en 1 Cachos dos Sonhos Multy Milagres (Multi Milagros). Crema de tratamiento, súper acondicionador y crema para peinar todo en uno. Proporciona antiporosidad y maciez extrema (suavidad extrema). Múltiples beneficios milagrosos para transformar los rizos. Presentación de 1kg.",
        caracteristicas: ["1kg", "3 en 1", "Antiporosidad", "Maciez extrema", "Multi beneficios"],
        imagen: "../img2/lote5/producto58.png"
    },
    {
        id: 59,
        nombre: "Cachos dos Sonhos Multy Poderosos 3em1",
        marca: "Salon Line (Línea Cachos dos Sonhos)",
        categoria: "cabello",
        descripcion: "Tratamiento 3 en 1 Cachos dos Sonhos Multy Poderosos (Multi Poderosos). Crema de tratamiento, súper acondicionador y crema para peinar en una sola fórmula. Tratamiento hidratante y brillo luminoso para rizos poderosos y saludables. Melancia (sandía). Presentación de 1kg.",
        caracteristicas: ["1kg", "3 en 1", "Hidratación poderosa", "Brillo luminoso", "Sandía"],
        imagen: "../img2/lote5/producto59.png"
    },
    {
        id: 60,
        nombre: "Kit Meu Liso Matizador Loiro - Shampoo y Acondicionador",
        marca: "Salon Line (Línea Meu Liso)",
        categoria: "cabello",
        descripcion: "Kit matizador para cabello rubio con shampoo y acondicionador cremoso. Neutraliza tonos amarillentos, anaranjados o decolorados. Matiza, cuida y proporciona brillo platinado. Fórmula profesional que mantiene el rubio perfecto y vibrante. Presentación de 300ml cada uno.",
        caracteristicas: ["Shampoo 300ml", "Acondicionador 300ml", "Matizador", "Para rubios", "Brillo platinado"],
        imagen: "../img2/lote5/producto60.png"
    },
    {
        id: 61,
        nombre: "#TodeCacho Máscara Matizadora Loira",
        marca: "Salon Line",
        categoria: "cabello",
        descripcion: "Máscara matizadora #TodeCacho para cabello rubio. Hidrata, trata, neutraliza y matiza en un solo paso. Elimina tonos amarillentos y anaranjados mientras proporciona tratamiento intensivo. Fórmula que mantiene el rubio perfecto con hidratación profunda. Presentación de 500g.",
        caracteristicas: ["500g", "Máscara matizadora", "Para rubios", "Hidrata y matiza", "Neutraliza amarillos"],
        imagen: "../img2/lote5/producto61.png"
    },
    {
        id: 62,
        nombre: "Cachos dos Sonhos Multy Extraordinário 3em1",
        marca: "Salon Line (Línea Cachos dos Sonhos)",
        categoria: "cabello",
        descripcion: "Tratamiento 3 en 1 Cachos dos Sonhos Multy Extraordinário (Multi Extraordinario). Crema de tratamiento, súper acondicionador y crema para peinar en una fórmula. Reparación intensiva y blindagem do frizz (blindaje del frizz). Con morango (fresa). Presentación de 1kg.",
        caracteristicas: ["1kg", "3 en 1", "Reparación intensiva", "Blindaje antifrizz", "Fresa"],
        imagen: "../img2/lote5/producto62.png"
    },
    {
        id: 63,
        nombre: "#TodeCacho Máscara Matizadora Vermelha",
        marca: "Salon Line",
        categoria: "cabello",
        descripcion: "Máscara matizadora #TodeCacho en tono rojo/vermelho. Hidrata, trata, neutraliza e intensifica tonos rojos. Revive el color, elimina decoloración y proporciona tratamiento profundo. Mantiene los tonos rojos vibrantes y brillantes por más tiempo. Presentación de 500g.",
        caracteristicas: ["500g", "Máscara matizadora", "Tono rojo", "Hidrata y matiza", "Intensifica color"],
        imagen: "../img2/lote5/producto63.png"
    },
    {
        id: 64,
        nombre: "Set Retinol & Ceramides - 4 Piezas",
        marca: "Nevada",
        categoria: "skincare",
        descripcion: "Set completo Nevada con Retinol y Ceramides para cuidado facial anti-edad. Incluye 4 piezas que combaten las arrugas, proporcionan firmeza y mejoran la textura de la piel. El retinol estimula la renovación celular mientras las ceramides fortalecen la barrera cutánea. Sistema completo profesional para una piel rejuvenecida y saludable.",
        caracteristicas: ["4 piezas", "Retinol", "Ceramides", "Anti-edad", "Renovación celular"],
        imagen: "../img2/lote5/producto64.jpg"
    },
    {
        id: 65,
        nombre: "Set Hyaluronic Acid - 3 Piezas",
        marca: "Nevada",
        categoria: "skincare",
        descripcion: "Set Nevada de Ácido Hialurónico con 3 piezas para hidratación profunda. Humecta y restaura la piel proporcionando hidratación intensa desde las capas más profundas. El ácido hialurónico retiene hasta 1000 veces su peso en agua, rellenando arrugas y proporcionando elasticidad. Sistema completo para una piel hidratada, suave y rejuvenecida.",
        caracteristicas: ["3 piezas", "Ácido Hialurónico", "Hidratación profunda", "Restauración", "Anti-arrugas"],
        imagen: "../img2/lote5/producto65.jpg"
    },
    {
        id: 66,
        nombre: "#TodeCacho Máscara Reparação Fortalecedora",
        marca: "Salon Line",
        categoria: "cabello",
        descripcion: "Máscara #TodeCacho de reparación fortalecedora. Tratamiento intensivo que repara y fortalece rizos dañados, debilitados o quebradizos. Reconstruye la fibra capilar, proporciona fuerza y definición. Ideal para rizos que necesitan reparación profunda. Presentación de 300g.",
        caracteristicas: ["300g", "Reparación", "Fortalecedora", "Para rizos", "Tratamiento intensivo"],
        imagen: "../img2/lote5/producto66.png"
    },
    {
        id: 67,
        nombre: "Set Whitening Skin - 3 Piezas Antimanchas",
        marca: "Nevada",
        categoria: "skincare",
        descripcion: "Set Nevada Whitening Skin de 3 piezas con Arbutina y Niacinamida para tratamiento antimanchas. Aclara, unifica el tono de la piel y reduce manchas oscuras, hiperpigmentación y decoloración. Fórmula avanzada que proporciona luminosidad y un tono de piel uniforme. Ideal para manchas por sol, edad o acné.",
        caracteristicas: ["3 piezas", "Arbutina", "Niacinamida", "Antimanchas", "Unifica tono"],
        imagen: "../img2/lote5/producto67.jpg"
    },
    {
        id: 68,
        nombre: "Set Aloe Vera - 3 Piezas",
        marca: "Nevada",
        categoria: "skincare",
        descripcion: "Set Nevada de Aloe Vera con 3 piezas para suavizar, hidratar y matificar la piel. El aloe vera proporciona hidratación natural, calma irritaciones y controla el brillo. Ideal para todo tipo de piel, especialmente piel sensible o con tendencia grasa. Proporciona frescura, equilibrio y textura sedosa.",
        caracteristicas: ["3 piezas", "Aloe Vera", "Hidratante", "Matificante", "Calma la piel"],
        imagen: "../img2/lote5/producto68.jpg"
    },
    {
        id: 69,
        nombre: "Set Vitamin C - 5 Piezas Anti-Edad",
        marca: "Nevada",
        categoria: "skincare",
        descripcion: "Set Nevada de Vitamina C con 5 piezas y Niacinamida para tratamiento anti-edad completo. Poderoso antioxidante que combate los signos de la edad, ilumina la piel, reduce manchas y estimula la producción de colágeno. Proporciona luminosidad, firmeza y protección contra el daño ambiental. Sistema profesional para una piel radiante.",
        caracteristicas: ["5 piezas", "Vitamina C", "Niacinamida", "Anti-edad", "Iluminador"],
        imagen: "../img2/lote5/producto69.jpg"
    },
    {
        id: 70,
        nombre: "Set Centella Asiática - 4 Piezas",
        marca: "Nevada",
        categoria: "skincare",
        descripcion: "Set Nevada de Centella Asiática con 4 piezas para reparación y calma de la piel. La centella asiática es conocida por sus propiedades cicatrizantes, antiinflamatorias y regeneradoras. Calma pieles sensibles, irritadas o con rojeces, mientras promueve la reparación y fortalecimiento de la barrera cutánea. Incluye banda para cabello.",
        caracteristicas: ["4 piezas", "Centella Asiática", "Reparadora", "Calma irritaciones", "Con banda"],
        imagen: "../img2/lote6/producto70.jpg"
    },
    {
        id: 71,
        nombre: "Mascarilla Facial Aloe Vera - Brillo Natural Hidratante",
        marca: "Nevada",
        categoria: "skincare",
        descripcion: "Mascarilla facial Nevada con aloe vera que proporciona brillo natural e hidratación profunda. El aloe vera calma, refresca y nutre la piel mientras proporciona hidratación intensa. Ideal para todo tipo de piel, especialmente piel sensible, irritada o deshidratada. Fórmula suave y natural que deja la piel radiante.",
        caracteristicas: ["1 unidad", "30g", "Aloe Vera", "Brillo natural", "Hidratante"],
        imagen: "../img2/lote6/producto71.jpg"
    },
    {
        id: 72,
        nombre: "Cápsulas Faciales Retinol & Coenzyme Q10 - Renueva y Suaviza",
        marca: "Nevada",
        categoria: "skincare",
        descripcion: "Cápsulas para cuidado facial y corporal Nevada con retinol y coenzima Q10. Fórmula anti-edad que renueva y suaviza la piel. El retinol estimula la renovación celular mientras la coenzima Q10 proporciona antioxidantes poderosos. Tratamiento intensivo que reduce arrugas, mejora textura y revitaliza la piel. 6 cápsulas monodosis.",
        caracteristicas: ["6 cápsulas", "Retinol", "Coenzima Q10", "Renueva y suaviza", "Rostro y cuerpo"],
        imagen: "../img2/lote6/producto72.jpg"
    },
    {
        id: 73,
        nombre: "Cápsulas Faciales Collagen & Hyaluronic Acid - Hidratación y Elasticidad",
        marca: "Nevada",
        categoria: "skincare",
        descripcion: "Cápsulas para cuidado facial y corporal Nevada con colágeno y ácido hialurónico. Proporciona hidratación profunda y elasticidad a la piel. El colágeno mejora la firmeza mientras el ácido hialurónico retiene la humedad. Tratamiento intensivo que rellena arrugas, mejora textura y proporciona juventud a la piel. 6 cápsulas monodosis.",
        caracteristicas: ["6 cápsulas", "Colágeno", "Ácido Hialurónico", "Hidratación", "Elasticidad"],
        imagen: "../img2/lote6/producto73.jpg"
    },
    {
        id: 74,
        nombre: "Mascarillas Contorno de Ojos Whitening Skin - Arbutina y Niacinamida",
        marca: "Nevada",
        categoria: "skincare",
        descripcion: "Mascarillas Nevada para contorno de ojos con arbutina y niacinamida para tratamiento antimanchas. Aclara ojeras, reduce manchas oscuras y unifica el tono alrededor de los ojos. Fórmula especializada que combate hiperpigmentación, proporciona luminosidad y rejuvenece la mirada. Incluye 20 pares (6g x 20 pares).",
        caracteristicas: ["20 pares", "6g por par", "Arbutina", "Niacinamida", "Antimanchas"],
        imagen: "../img2/lote6/producto74.jpg"
    },
    {
        id: 75,
        nombre: "Set Serum Nevada - Biotina, Ricino y Romero",
        marca: "Nevada",
        categoria: "cabello",
        descripcion: "Set de 3 sérums capilares Nevada. Incluye Biotina con Romero (fortalece y estimula crecimiento), Ricino/Castor Oil (nutrición y crecimiento), y Aceite de Ricino con vitaminas (fuerza, prevención de caída y crecimiento). Sistema completo para fortalecer, nutrir y estimular el crecimiento del cabello.",
        caracteristicas: ["Set 3 sérums", "Biotina", "Aceite de Ricino", "Romero", "Crecimiento capilar"],
        imagen: "../img2/lote6/producto75.png"
    },
    {
        id: 76,
        nombre: "Mascarillas Contorno de Ojos Vitamin C - Vitamina E",
        marca: "Nevada",
        categoria: "skincare",
        descripcion: "Mascarillas Nevada para contorno de ojos con vitamina C y vitamina E. Tratamiento iluminador y antioxidante que reduce ojeras, combate signos de fatiga y proporciona luminosidad. Las vitaminas C y E protegen contra radicales libres, mejoran la circulación y revitalizan la piel delicada del contorno de ojos. Incluye 20 pares (6g x 20 pares).",
        caracteristicas: ["20 pares", "6g por par", "Vitamina C", "Vitamina E", "Iluminador"],
        imagen: "../img2/lote6/producto76.jpg"
    },
    {
        id: 77,
        nombre: "Mascarillas Contorno de Ojos Hyaluronic Acid",
        marca: "Nevada",
        categoria: "skincare",
        descripcion: "Mascarillas Nevada para contorno de ojos con ácido hialurónico. Hidratación profunda que rellena arrugas, reduce bolsas y proporciona elasticidad al área de los ojos. El ácido hialurónico retiene la humedad, mejora la firmeza y suaviza líneas de expresión. Tratamiento intensivo para una mirada rejuvenecida. Incluye 20 pares (6g x 20 pares).",
        caracteristicas: ["20 pares", "6g por par", "Ácido Hialurónico", "Hidratación profunda", "Anti-arrugas"],
        imagen: "../img2/lote6/producto77.jpg"
    },
    {
        id: 78,
        nombre: "Stem Cells Facial Tonic - Limpiador Facial de Células Madres",
        marca: "Nevada",
        categoria: "skincare",
        descripcion: "Tónico facial Nevada con células madres para limpieza profunda. Limpia, purifica y proporciona frescura mientras estimula la renovación celular. Las células madres revitalizan la piel, mejoran textura y promueven una piel radiante. Efecto antibriosa (matificante) para piel radiante. Presentación de 180ml / 6oz.",
        caracteristicas: ["180ml", "Células Madres", "Limpiador facial", "Purifica", "Efecto matificante"],
        imagen: "../img2/lote6/producto78.jpg"
    },
    {
        id: 79,
        nombre: "Cápsulas Faciales Aloe Vera & Vitamin E - Suaviza y Revitaliza",
        marca: "Nevada",
        categoria: "skincare",
        descripcion: "Cápsulas para cuidado facial y corporal Nevada con aloe vera y vitamina E. Suaviza y revitaliza la piel proporcionando hidratación natural y protección antioxidante. El aloe vera calma mientras la vitamina E regenera. Tratamiento que nutre, repara y proporciona suavidad extrema. 6 cápsulas monodosis.",
        caracteristicas: ["6 cápsulas", "Aloe Vera", "Vitamina E", "Suaviza", "Revitaliza"],
        imagen: "../img2/lote6/producto79.jpg"
    },
    {
        id: 80,
        nombre: "Facial Mask Naranja con Ácido Hialurónico - Exfoliante Energizante",
        marca: "Nevada",
        categoria: "skincare",
        descripcion: "Mascarilla facial Nevada de naranja con ácido hialurónico. Tratamiento exfoliante y energizante para pieles grasas. La naranja proporciona vitamina C y antioxidantes que iluminan, mientras el ácido hialurónico hidrata profundamente. Exfolia suavemente, energiza la piel y proporciona luminosidad. Kit incluye mascarilla y serum. 10 unidades.",
        caracteristicas: ["10 unidades", "Naranja", "Ácido Hialurónico", "Exfoliante", "Para piel grasa"],
        imagen: "../img2/lote6/producto80.jpg"
    },
    {
        id: 81,
        nombre: "Mascarillas Contorno de Ojos Aloe Vera",
        marca: "Nevada",
        categoria: "skincare",
        descripcion: "Mascarillas Nevada para contorno de ojos con aloe vera. Calma, hidrata y refresca el área delicada de los ojos. El aloe vera proporciona hidratación natural, reduce inflamación y alivia irritaciones. Ideal para ojos cansados, hinchados o con ojeras. Tratamiento relajante que revitaliza la mirada. Incluye 20 pares (6g x 20 pares).",
        caracteristicas: ["20 pares", "6g por par", "Aloe Vera", "Calma e hidrata", "Anti-inflamatorio"],
        imagen: "../img2/lote6/producto81.jpg"
    },
    {
        id: 82,
        nombre: "Facial Mask Barro Marino con Ácido Hialurónico - Purificante",
        marca: "Nevada",
        categoria: "skincare",
        descripcion: "Mascarilla facial Nevada de barro marino con ácido hialurónico. Tratamiento purificante y revitalizante para pieles grasas. El barro marino limpia profundamente los poros, elimina impurezas y controla el exceso de grasa, mientras el ácido hialurónico hidrata. Desintoxica, purifica y equilibra la piel. 10 unidades.",
        caracteristicas: ["10 unidades", "Barro Marino", "Ácido Hialurónico", "Purificante", "Para piel grasa"],
        imagen: "../img2/lote6/producto82.jpg"
    },
    {
        id: 83,
        nombre: "Black Mask Whitening Fresa - Aclarador Facial Anti-Edad Toner",
        marca: "Nevada",
        categoria: "skincare",
        descripcion: "Mascarilla negra Nevada con extracto de fresa para aclarar y rejuvenecer. Fórmula whitening anti-edad con tóner que aclara manchas, unifica el tono y proporciona luminosidad. La fresa aporta antioxidantes y vitamina C. Ideal para pieles normales. Limpia profundamente, exfolia suavemente y aclara. Presentación de 120g / 4oz.",
        caracteristicas: ["120g", "Extracto de Fresa", "Whitening", "Anti-edad", "Tóner"],
        imagen: "../img2/lote6/producto83.jpg"
    }
];